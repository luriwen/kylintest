...
num=`grep  "CONFIG_" .config | grep y | wc -l`
if [ $num -gt 0 ]; then
	...
	do
		...
		if [ $option == CONFIG_UDISK ]; then
			echo "				U盘拷贝测试:				"
			Diskcptest

		elif [ $option == CONFIG_UNIXBENCH ]; then
			echo "				Unixbench测试:"
			UnixBench

		elif [ $option == CONFIG_STREAM ]; then
			echo "				STREAM测试:"
			STREAM

		elif [ $option == CONFIG_IOZONE ]; then
			echo "				iozone测试:"
			Iozone

		elif [ $option == CONFIG_LMBENCH ]; then
			echo "				lmbench测试:"
			Lmbench

		elif [ $option == CONFIG_IPERF ]; then
			echo "				iperf测试:"
			Iperf

		elif [ $option == CONFIG_SPECJVM ]; then
			echo "				specjvm测试:"
			Specjvm

		elif [ $option == CONFIG_TTYTEST ]; then
			echo "				串口测试:"
			Ttytest
		fi
	done
